...
## Plot the map
map <- get_map(c(4.45, 52), zoom = 8, source = 'stamen', maptype = 'watercolor')               
p <- ggmap(map) 
...
p <- p + geom_text(data = nl.selection, aes(x=long, y=lat, label=name, family='serif', size=4), check_overlap = TRUE, angle = 30, show.legend = FALSE)
p <- p + geom_text(data = be.selection, aes(x=long, y=lat, label=name, family='serif', size=4), check_overlap = TRUE, angle = 30, show.legend = FALSE)
